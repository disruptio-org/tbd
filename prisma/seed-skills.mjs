/**
 * Seed script: Assistant Skills
 * Uses Supabase Data API (same approach as seed-http.mjs)
 * Run with: node prisma/seed-skills.mjs
 */
...
const getEnv = (key) => env.match(new RegExp(`${key}="([^"]+)"`))?.[1] || env.match(new RegExp(`${key}=([^\r\n]+)`))?.[1];
...
// ─── Default skills extracted from the assistant pages ──────────
...
async function main() {
    ...
    // Get all companies
    const { data: companies, error: compError } = await db.from('Company').select('id, name');
    if (compError) { console.error('❌ Cannot fetch companies:', compError.message); process.exit(1); }
    ...
    for (const company of companies) {
        ...
        // Upsert: use onConflict on the unique constraint columns
        const { error } = await db
            .from('AssistantSkill')
            .upsert(rows, { onConflict: 'companyId,assistantType,key', ignoreDuplicates: true });
