...
export default function GeneralChatPage() {
    ...
    const { skills: contentTypes } = useAssistantSkills('GENERAL_AI');
    ...
    const loadHistory = useCallback(() => {
        fetch('/api/general-ai/history').then(r => r.json()).then(d => setHistory(d.runs || [])).catch(() => {});
    }, []);
    ...
    async function handleGenerate(params: Record<string, unknown>, refinement?: { action: string; previousOutput: string }) {
        ...
        try {
            const payload: Record<string, unknown> = {
                contentType: params.contentType || 'RESEARCH_SUMMARY',
            ...
            const res = await fetch('/api/general-ai/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    async function handleViewHistory(id: string) {
        const res = await fetch(`/api/general-ai/history/${id}`); const data = await res.json();
    ...
    function handleDeleteHistory(id: string) {
        showConfirm('Delete this record?', async () => { await fetch('/api/general-ai/history', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); setHistory(h => h.filter(x => x.id !== id)); showToast('Deleted', 'success'); });
    }

    function handleExport(content: string, title: string) {
        const ct = result?.contentType || 'RESEARCH_SUMMARY';
        ...
        const md = [
            ...
            `> **Powered by:** Nousio AI Assistant`,
            '',
            '---',
            '',
            content,
        ...
        <div className="mkt-page">
            ...
            {tab === 'create' && (
                <AIAssistantChat
                    ...
                    headerIcon={<Bot size={18} strokeWidth={2} />}
