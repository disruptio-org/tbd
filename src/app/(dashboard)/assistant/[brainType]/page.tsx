...
import { useAssistantSkills, skillLabel } from '@/lib/useAssistantSkills';
import {
    Brain,
    Sparkles, Clock, Play, Trash2, Copy, Download, Eye, Plus,
    Building2, Coins, Megaphone, GraduationCap, Target, Package, Rocket,
} from 'lucide-react';
...
const SECTION_ORDER: [string, string][] = [
    ['hook', 'Overview'], ['headline', 'Title'], ['body', 'Content'], ['sections', 'Sections'],
];

const LOADING_STEPS = ['Loading company context...', 'Analyzing your request...', 'Generating content...', 'Refining output...'];
...
interface BrainInfo { id: string; brainType: string; name: string; description: string | null; }
...
export default function DynamicAssistantPage() {
    const params = useParams();
    const brainType = (params.brainType as string || '').toUpperCase();
    ...
    const { skills: contentTypes } = useAssistantSkills(brainType);
    function typeLabel(value: string) { return skillLabel(contentTypes, value); }
    ...
    useEffect(() => {
        fetch('/api/ai/brains')
            .then(r => r.json())
            .then(data => {
                const found = (data.brains || []).find((b: BrainInfo) => b.brainType === brainType);
                if (found) setBrainInfo(found);
            })
            .catch(() => {});
    }, [brainType]);

    const loadHistory = useCallback(() => {
        fetch(`/api/assistant/${brainType.toLowerCase()}/history`).then(r => r.json()).then(d => setHistory(d.runs || [])).catch(() => {});
    }, [brainType]);
    ...
    async function handleGenerate(params: Record<string, unknown>, refinement?: { action: string; previousOutput: string }) {
        ...
        try {
            ...
            const res = await fetch(`/api/assistant/${brainType.toLowerCase()}/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            const data = await res.json();
            if (!res.ok) { showToast(data.error || 'Error generating content', 'error'); return; }
            setResult(data); showToast('Content generated!', 'success');
    ...
    async function handleViewHistory(id: string) {
        const res = await fetch(`/api/assistant/${brainType.toLowerCase()}/history/${id}`); const data = await res.json();
    ...
    function handleDeleteHistory(id: string) {
        showConfirm('Delete this record?', async () => { await fetch(`/api/assistant/${brainType.toLowerCase()}/history`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); setHistory(h => h.filter(x => x.id !== id)); showToast('Deleted', 'success'); });
    }

    function handleExport(content: string, title: string) {
        const ct = result?.contentType || 'GENERAL';
        const typeName = typeLabel(ct);
        const date = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        const displayName = brainInfo?.name || brainType.replace(/_/g, ' ');
        ...
        const md = [
            ...
            `> **Powered by:** ${displayName} — Nousio AI`,
            '',
            '---',
            '',
            content,
    ...
    const displayName = brainInfo?.name || brainType.replace(/^CUSTOM_/, '').replace(/_/g, ' ');
    const headerIcon = <Brain size={20} strokeWidth={2} />;
        ...
        <div className="mkt-page">
            ...
            <div className="assistant-page-header">
                <div className="assistant-page-title">
                    <span className="assistant-page-icon">{headerIcon}</span>
                    <h1>{displayName}</h1>
                </div>
