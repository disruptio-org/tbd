...
const SECTION_ORDER: [string, string][] = [
    ['hook', 'Introduction'], ['headline', 'Title'], ['body', 'Content'], ['sections', 'Sections'],
];

const LOADING_STEPS = ['Loading company context...', 'Analyzing onboarding needs...', 'Generating content...', 'Refining for new hires...'];
...
export default function OnboardingAssistantPage() {
    ...
    const { skills: contentTypes } = useAssistantSkills('ONBOARDING');
    ...
    const loadHistory = useCallback(() => {
        fetch('/api/onboarding-assistant/history').then(r => r.json()).then(d => setHistory(d.runs || [])).catch(() => {});
    }, []);
    ...
    async function handleGenerate(params: Record<string, unknown>, refinement?: { action: string; previousOutput: string }) {
        ...
        try {
            const payload: Record<string, unknown> = {
                contentType: params.contentType || 'WELCOME_GUIDE',
            ...
            const res = await fetch('/api/onboarding-assistant/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    async function handleViewHistory(id: string) {
        const res = await fetch(`/api/onboarding-assistant/history/${id}`); const data = await res.json();
    ...
    function handleDeleteHistory(id: string) {
        showConfirm('Delete this record?', async () => { await fetch('/api/onboarding-assistant/history', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); setHistory(h => h.filter(x => x.id !== id)); showToast('Deleted', 'success'); });
    }

    function handleExport(content: string, title: string) {
        const ct = result?.contentType || 'WELCOME_GUIDE';
        ...
        const md = [
            ...
            `> **Powered by:** Nousio Onboarding Assistant`,
            '',
            '---',
            '',
            content,
        ...
        <div className="mkt-page">
            ...
            <div className="assistant-page-header">
                <div className="assistant-page-title">
                    <span className="assistant-page-icon"><GraduationCap size={20} strokeWidth={2} /></span>
            ...
            {tab === 'create' && (
                <AIAssistantChat
                    ...
                    headerIcon={<GraduationCap size={18} strokeWidth={2} />}
            ...
            {tab === 'history' && (
                <div className="assistant-history-content">
                    <div className="assistant-history-header">
                        <h2>HISTORY</h2>
                        <p>Continue previous work or review past onboarding content</p>
                    </div>
